...
$is_effect = (isset($boset['effect']) && $boset['effect']) ? true : false;
...
<div class="list-board">
	<div class="div-head <?php echo $head_class;?>">
		...
		<?php } ?>
		<?php if($is_effect) { ?>
			<span class="wr-effect hidden-xs"><?php echo subject_sort_link('effect', $qstr2, 1) ?>결과</a></span>
		<?php } ?>
	</div>
	<ul class="list-body">
	<?php
	...
	?>
		<li class="list-item<?php echo $li_css;?>">
			...
			<div class="wr-subject">
				...
				<?php if(!$list[$i]['is_notice']) { //공지가 아닐경우 ?>
					<div class="item-details text-muted font-12 visible-xs ellipsis">
						...
						<?php } ?>
						<?php if($is_effect) { ?>
							<span<?php echo ($list[$i]['effect'])?' class="blue"':' class="red"';?>><i class="fa fa-asterisk"></i> <?php echo ($list[$i]['effect'])?'접수완료':'접수중';?></span>
						<?php } ?>
						<span>
			...
			<?php } ?>
			<?php if($is_effect) { ?>
				<div class="wr-effect hidden-xs">
					<span class="wr-text <?php echo ($list[$i]['effect'])?'blue':'red';?>"><?php echo ($list[$i]['effect'])?'접수완료':'접수중';?></span>
				</div>
			<?php } ?>
		</li>
	<?php } ?>
	...
	<?php if (!$is_list) { ?>
		<div class="wr-none">상담정보가 없습니다.</div>
	<?php } ?>
</div>
