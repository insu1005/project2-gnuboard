...
<div id="viewcomment">
	<div class="view-comment font-18 en">
		<i class="fa fa-commenting"></i> <span class="orangered"><?php echo $write['wr_comment'];?></span> Comments
		<?php if($is_cmt_star) { 
			$vstar = apms_post_star($write, $is_cmt_star);
		?>
			&nbsp;
			<?php echo $vstar['star'];?> 
			&nbsp;
			<?php echo sprintf("%2.1f", $vstar['score']);?> 
			<span class="lightgray">/</span> 
			<i class="fa fa-user gray"></i> <?php echo $vstar['cnt'];?>
		<?php } ?>
	</div>
	<?php
	// 댓글이 있으면
				...
				<div class="media" id="c_<?php echo $comment_id ?>"<?php echo ($cmt_depth) ? ' style="margin-left:'.$cmt_depth.'px;"' : ''; ?>>
					...
					<div class="media-body">
						<div class="media-heading">
							<?php if($is_cmt_star && $list[$i]['as_star_score']) { ?>
								<?php echo apms_get_star($list[$i]['as_star_score'], 'fa-lg '.$is_cmt_star);?>
								&nbsp;
							<?php } ?>
