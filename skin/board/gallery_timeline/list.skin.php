...
add_stylesheet('<link rel="stylesheet" href="'.$board_skin_url.'/css/style.css">', 1);
...
?>
	...
	<?
		$k=1;
		for($i=1; $i <= $max_row; $i+=2){
			if($i != 1)
				$max_row_hegit += 355;	
			if($i == 5)	
				$max_row_hegit += 10;
	?>
			.demo-card:nth-child(<?=$i?>) {
			  order: <?=$k?>;
			}
	<?	
			$k++;
		}
		for($i=2; $i <= $max_row; $i+=2){
			$max_row_hegit += 140;
	?>
			.demo-card:nth-child(<?=$i?>) {
			  order: <?=$k?>;
...
<div class="inner clearfix">
	...
	<?
		for($i=0; $i<$max_row; $i++){
			// $row_num = ($i+1)%5 == 0 ? 5 : ($i+1)%5;
			$row_num = mt_rand(1,5);
	?>	
			<div class="demo-card demo-card--step<?=$row_num?>">
				<div class="head">
					<div class="date-box" >
						<div class="demo-btn demo-card--step<?=$row_num?>">
							<i class="fa fa-clock-o" aria-hidden="true"></i> <?=$list[$i]['datetime']?><br><?=substr($list[$i]['wr_datetime'],11,5)?>
						</div>
					</div>
					<div class="t-title">
						...
						<div class="gall_name" >
	                        ...
	                        ?>
	                        	<a href="<?php echo $list[$i]['ca_name_href'] ?>" class="bo_tit">[<?php echo $list[$i]['ca_name'] ?>]</a> 
	                        <?} 
				...
				<div class="body">
					...
					<div class="body-text" >
						<?=remove_tag($list[$i]['wr_content'], array('img','a'))?>
					 </div>
