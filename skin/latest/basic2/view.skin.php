...
<!-- 이미지 -->
<style type="text/css">
#main {position:relative;width:100%;background:#ddd;padding:10px;margin-bottom:15px;}
#featured-content {width:100%;background:#6E6E6E;position:relative;height:600px;text-align:center;overflow:hidden;}
.featured-post {float:left;position:relative;padding:10px 0;height:600px;overflow:hidden;}
.slider-nav {display:block;position:absolute;top:50%;width:40px;height:50px;background:#888;color:#fff;font-size:2.5em;z-index:9999999;cursor:pointer;}
#slider-prev {left:0;background-position:7px -21px;padding-right:5px;border-radius:0 20px 20px 0;}
#slider-next {right:0;background-position:7px 11px;padding-left:7px;border-radius:20px 0 0 20px;}
.slider-nav:hover {background-color:#000;}
#slider-nav {display:block;overflow:hidden;padding:0;background:#6E6E6E;}
#slide-thumbs {width:100%;height:70px;margin:0;}
#slider-nav li {float:left;width:10%;height:70px;}
#slider-nav li img{width:100%;height:100%;object-fit:cover;}
#slider-nav li video{width:100%;height:100%;object-fit:cover;}
</style>
<script type="text/javascript" src="<?php echo $board_skin_url; ?>/js/jquery.js"></script>
<script type="text/javascript" src="<?php echo $board_skin_url; ?>/js/jquery.masonry.min.js"></script>
<script type="text/javascript" src="<?php echo $board_skin_url; ?>/js/jquery.cycle.min.js"></script>
<!-- 이미지 -->
...
<div id="main">
  <div class="featured-wrapper">
    <div id="featured-content">
      <?php
      $image_source = $view['content'];
      preg_match_all("/<img[^>]*src=[\'\"]?([^>\'\"]+)[\'\"]?[^>]*>/", $image_source, $img);
      ?>
      <?php
      foreach ($img[1] as $item) {
	  ?>
      <div class="featured-post"><a href="<?php echo $item ?>" target="_blank" class="view_image" title="크게보기 클릭"><img src="<?php echo $item ?>" style="max-width:100%;max-height:100%;"></a></div>
      <?php } ?>
    ...
    <div id="slider-nav">
      <ul id="slide-thumbs">
        ...
        foreach ($img[1] as $item) {
	    ?>
		<li><a href="flat-design-fonts"><img src="<?php echo $item ?>" style="max-width:100%;max-height:100%;"></a></li>
        <?php } ?>
      </ul>
	</div>
  </div>
...
var slider_settings = {"timeout":"99999999999"}; // 자동 슬라이드 숫자를 낮추세요.
</script>
<script type="text/javascript" src="<?php echo $board_skin_url; ?>/js/footer-scripts-light.js"></script>
<!-- 이미지 -->
...
$(function() {
    $("a.view_image").click(function() {
        window.open(this.href, "large_image", "location=yes,links=no,toolbar=no,top=10,left=10,width=auto,height=auto,resizable=yes,scrollbars=no,status=no");
        return false;
    });
